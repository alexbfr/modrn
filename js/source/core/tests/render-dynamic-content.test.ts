...
import {useChild, useChildren, useTemplate} from "../templated-children-hooks";
import {clearRenderQueue, getRenderQueueLength, renderElements, requestRender} from "../render-queue";
...
it("Correctly re-renders a collection of templated dynamic children", async () => {
    ...
    const container = await TestUtils.render("div") as HTMLElement;
    container.innerHTML = `<p>Prologue</p>${tag1.tagOpen("id='123'")}--{{item}}--${tag1.tagClose}<p>Epilogue</p>`;
