...
function extractExpression(textContent) {
    ...
    if (!textContent.startsWith("{{")) {
        return {
            expressionType: exports.ExpressionType.ConstantExpression,
            value: textContent
        };
    }
...
function extractVariableReferenceExpression(text) {
    return { expressionType: exports.ExpressionType.VariableUsage, variableName: text };
}
function extractComplexExpression(text) {
    ...
    return {
        expressionType: exports.ExpressionType.ComplexExpression,
...
function extractFunctionReferenceExpression(text) {
    ...
    const compiled = compile(expression);
    return {
        expressionType: exports.ExpressionType.FunctionReferenceExpression,
        usedVariableNames: variableNames,
        expression,
        compiledExpression: compiled,
        originalExpression: "&" + text
...
function findChildVariables(rootElement, indexes) {
    ...
    for (let idx = 0; idx < length; ++idx) {
        ...
        if (textContent && textContent.startsWith("{{") && textContent.endsWith("}}")) {
            ...
            result.push({
                expression,
                type: exports.MappingType.childVariable,
                indexes: newIndexes
            });
        }
    }
        ...
        for (let attIdx = 0; attIdx < attributesLength; ++attIdx) {
            ...
            if (name && value && (classification === null || classification === void 0 ? void 0 : classification.type) === "standard" && expressionPattern.test(value)) {
                ...
                result.push({
                    ...
                    indexes,
                    hidden: false,
                    type: exports.MappingType.attribute
                });
            }
        }
    }
    ...
    if (attributes && (attributes === null || attributes === void 0 ? void 0 : attributes.length) > 0) {
        for (let attIdx = 0; attIdx < attributesLength; ++attIdx) {
            ...
            if (name && value && classification.type === "ref") {
                ...
                result.push({
                    expression,
                    type: exports.MappingType.attributeRef,
                    indexes
                });
            }
        }
...
function findVariables(rootElement) {
    ...
    vars.forEach(varMapping => {
        ...
        if (varMapping.expression.expressionType === exports.ExpressionType.ConstantExpression) {
...
function allVariableReferencesOf(varMapping) {
    switch (varMapping.expression.expressionType) {
        case exports.ExpressionType.VariableUsage:
            return [varMapping.expression.variableName];
        case exports.ExpressionType.ComplexExpression:
            return varMapping.expression.usedVariableNames;
        case exports.ExpressionType.ConstantExpression:
            return [];
        case exports.ExpressionType.FunctionReferenceExpression:
            return varMapping.expression.usedVariableNames;
        default:
    ...
    if (specialAttributeHandlerResult.transformedElement && specialAttributeHandlerResult.transformedElement !== rootElement) {
        ...
        if (subFragment.variableDefinitions) {
            const subVariables = Object.keys(subFragment.variableDefinitions.all)
                ...
                type: exports.MappingType.attribute,
                hidden: true,
                expression: { expressionType: exports.ExpressionType.VariableUsage, variableName }
            }));
...
function substituteVariables(self, root, varsProvided, variableDefinitionsProvided, suppressReRender = false) {
    ...
    variableDefinitions.sorted.forEach(variableDefinitionForNode => {
        ...
        Object.keys(vars).forEach(variableName => {
            ...
            for (const match of matches) {
                let value;
                if (match.expression.expressionType === exports.ExpressionType.VariableUsage) {
                    // Just variable reference
                    value = vars[variableName];
                }
                else if (match.expression.expressionType === exports.ExpressionType.ComplexExpression) {
                    // Complex expression
                    try {
                ...
                else if (match.expression.expressionType === exports.ExpressionType.FunctionReferenceExpression) {
                    // Function reference
    ...
    function setValue(node, match, valueProvided, slotsBySpecialAttribute, specialAttributeProcessFns) {
        ...
        switch (match.type) {
            case exports.MappingType.attributeRef: {
            ...
            case exports.MappingType.specialAttribute: {
            ...
            case exports.MappingType.attribute: {
                const attributeVariable = match;
            ...
            case exports.MappingType.childVariable: {
...
exports.componentHasDisconnected = componentHasDisconnected;
exports.componentStaticInitialize = componentStaticInitialize;
exports.createChangeHook = createChangeHook;
exports.createChildrenState = createChildrenState;
...
exports.mBool = mBool;
exports.mChild = mChild;
exports.mDate = mDate;
exports.mEventHandler = mEventHandler;
exports.mFunction = mFunction;
...
exports.mutableStateInternal = mutableStateInternal;
exports.only = only;
exports.purify = purify;
exports.purifyInternal = purifyInternal;
...
exports.useStateInternal = useStateInternal;
exports.useTemplate = useTemplate;
exports.useTemplatedChildren = useTemplatedChildren;
exports.varsWithOptions = varsWithOptions;
 ...
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.todoMvcModule = void 0;
const modrn_1 = require("modrn");
const pluralize = (what) => what.length === 1 ? "item" : "items";
const headerComponentProps = modrn_1.m({
    addTodo: modrn_1.mFunction()
});
const headerComponentState = modrn_1.createState();
const headerComponent = modrn_1.makeComponent(headerComponentProps, ({ addTodo }) => {
    const [{ newTodo }] = modrn_1.useState(headerComponentState, { newTodo: "" });
    const setText = modrn_1.purify(headerComponentState, (state, evt) => { var _a; return ({ newTodo: (_a = evt === null || evt === void 0 ? void 0 : evt.target) === null || _a === void 0 ? void 0 : _a.value }); });
    const onEnter = modrn_1.purify(headerComponentState, (state) => {
...
const todoListProps = modrn_1.m({
    visibility: modrn_1.mString(),
    todos: modrn_1.mObj(),
    updateTodo: modrn_1.mFunction(),
    removeTodo: modrn_1.mFunction(),
    removeCompleted: modrn_1.mFunction()
});
const editTodoState = modrn_1.createState();
const todoListComponent = modrn_1.makeComponent(todoListProps, ({ visibility, todos, updateTodo, removeTodo }) => {
    const [editTodo, setEditTodo] = modrn_1.useState(editTodoState, {});
    ...
    function toggleAllDone() {
        todos.forEach(todo => updateTodo(Object.assign(Object.assign({}, todo), { completed: !allDone })));
    }
    function toggleCompleted(todo) {
        updateTodo(Object.assign(Object.assign({}, todo), { completed: !todo.completed }));
    }
    function onEdit(todo) {
        setEditTodo({ todo: Object.assign({}, todo) });
    }
    const finishEdit = modrn_1.purify(editTodoState, ({ todo }, newTitle) => {
        if (todo) {
    ...
    function onFinish(evt) {
        var _a;
        finishEdit((_a = evt.target) === null || _a === void 0 ? void 0 : _a.value);
    }
    function onEscape() {
        setEditTodo({});
    }
    function isEditing(todo) {
        var _a;
        return todo.id === ((_a = editTodo.todo) === null || _a === void 0 ? void 0 : _a.id);
    }
...
`)
    .register();
const footerComponent = modrn_1.makeComponent(todoListProps, ({ visibility, todos, removeCompleted }) => {
...
`)
    ...
    .register();
const todoState = modrn_1.createState();
let nextId = 0;
...
const hashChangeEventListener = modrn_1.createEventListener();
const todoMvcComponent = modrn_1.makeComponent(modrn_1.NoProps, () => {
    const [{ todos, visibility }] = modrn_1.useLocalStorageState(todoState, { visibility: getVisibility(), todos: [] }, "todos-modrnts");
    modrn_1.useEventListener(hashChangeEventListener, window, "hashchange", modrn_1.purify(todoState, (state) => ({ visibility: getVisibility() })));
    const addTodo = modrn_1.purify(todoState, (state, newTodo) => ({ todos: [...state.todos, { id: nextId++, title: newTodo, completed: false }] }));
    const updateTodo = modrn_1.purify(todoState, (state, todo) => {
        const newTodos = state.todos.map(current => current.id === todo.id ? todo : current);
        return { todos: newTodos };
    });
    const removeTodo = modrn_1.purify(todoState, (state, todo) => {
        return { todos: state.todos.filter(current => current.id !== todo.id) };
    });
    const removeCompleted = modrn_1.purify(todoState, (state) => {
...
`)
    .register();
exports.todoMvcModule = modrn_1.declare({ todoMvcComponent, headerComponent, todoListComponent, footerComponent });
modrn_1.modrn(exports.todoMvcModule);
